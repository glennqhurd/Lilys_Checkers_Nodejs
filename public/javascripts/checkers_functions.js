...
let occupiedArray = new Array(8);
for (let k = 0; k < 8; k++) {
    occupiedArray[k] = [];
}
let checkerArray = new Array(48);
let clickedCheckerId = null;
let savedBoard = "";
let turnCounter = 1;
let hasMoved = false;
let select = document.getElementById("gameNumbers");
let move_dict = {
    "B8" : 1,
    "D8" : 2,
    "F8" : 3,
...
function toggleMoveButton() {
    document.getElementById("promptButton").disabled = !(document.getElementById("redCBox").checked && currentColor === "red" || document.getElementById("blackCBox").checked && currentColor === "black");
...
function getComputersMove() {
    let boardString = combineIntoURL();
...
function validDifficulty() {
    return (document.getElementById('difficultySetting').value >= 1) && (document.getElementById('difficultySetting').value <= 10);
}

function combineIntoURL() {
    let boardString = getBoard();
    let difficulty = document.getElementById('difficultySetting').value;
    let player = boardString[0];
    let board = boardString.slice(2);
...
function sendCheckersEmail() {
    if (checkSendInfo()) {
        ...
        let e = document.getElementById("gameNumbers");
        let gameNumber = parseInt(e.options[e.selectedIndex].text);
        let user = document.getElementById("userName").value;
        let password = document.getElementById("emailPassword").value;
        let move = document.getElementById("moveRecord").innerHTML;
    ...
    if (document.getElementById("userName").value !== "") {
        ...
        xhttp.onreadystatechange = function() {
            ...
            if (this.readyState === 4 && this.status === 200) {
                ...
                document.getElementById("findNumbersButton").disabled = false;
                let gameDict = JSON.parse(this.responseText);
...
function resetBoard() {
    ...
    let boardString = "b:bbbbbbbbbbbb--------rrrrrrrrrrrr";
    setBoard(boardString);
...
function clearBoard() {
    ...
    document.getElementById("boardInput").value = "b:bbbbbbbbbbbb--------rrrrrrrrrrrr";
    //document.getElementById("blackCBox").checked = false;
    //document.getElementById("redCBox").checked = false;
    document.getElementById("forcedJump").innerHTML = "No forced jumps.";
    document.getElementById("promptButton").disabled = true;
...
function setBoard(boardString) {
    ...
    for (let i = 2; i < boardString.length; i++) {
    ...
    for (let i = 2; i < boardString.length; i++) {
        let floorRow = Math.floor((i - 2)/4);
        let floorCol = ((i - 2) % 4);

        if (boardString.charAt(i) === "b") {
            checkerId = "black" + blackCount;
            blackCount++;
        }
        else if (boardString.charAt(i) === "r") {
            checkerId = "red" + redCount;
            redCount++;
        }
        else if (boardString.charAt(i) === "B") {
            checkerId = "bking" + blackKingCount;
            blackKingCount++;
        }
        else if (boardString.charAt(i) === "R") {
            checkerId = "rking" + redKingCount;
            redKingCount++;
        }
        else {
            checkerId = null;
    ...
    toggleMoveButton();
    clearSpaces();
    let jumpList = jumpExists(currentColor);
    return true;
}
...
function drop(event) {
    clearSpaces();
    let checkerId = event.dataTransfer.getData("Text");
    let destRow = Math.floor((event.pageY - 7 - document.getElementById(checkerId).parentElement.offsetTop) / 50);
    let destCol = Math.floor((event.pageX - 7 - document.getElementById(checkerId).parentElement.offsetLeft) / 50);
    ...
    if (isLegalMove(destRow, destCol, checkerId, currentColor)) {
        ...
        let winner = checkForWinner();
        let previousBoard = document.getElementById("boardInput").value;
...
function boardClick(event) {
    clearSpaces();
    let destCol = Math.floor((event.pageX - document.getElementById(clickedCheckerId).parentElement.offsetLeft) / 50);
    let destRow = Math.floor((event.pageY - document.getElementById(clickedCheckerId).parentElement.offsetTop) / 50);
...
function kingAPiece(row, col, checkerId) {
    if (!document.getElementById(checkerId).classList.contains("king")) {
        let index = findCheckerIndex(checkerId);
...
function isLegalMove(row, col, checkerId, color) {
    ...
    return !(canJump(srcRow, srcCol) && !hasOppositeChecker((row + (srcRow - row) / 2), (col + (srcCol - col) / 2), currentColor));
}
...
function makeJumpMove(destRow, destCol, checkerId, color) {
    ...
    let jumpedId = occupiedArray[destRow + (srcRow - destRow)/2][destCol + (srcCol - destCol)/2].id;
...
function checkAdjacent(row, col, checkerId, color) {
    ...
    if (isAKing(checkerId)) {
        if (!outOfBounds(row + 2, col + 2) && (!cellIsVacant(row + 1, col + 1)) && (cellIsVacant(row + 2, col + 2))) {
            adjacentId = occupiedArray[row + 1][col + 1].id;
...
function jumpExists(color) {
    ...
    for (let i = 0; i < occupiedArray.length; i++) {
        for (let j = 0; j < occupiedArray[i].length; j++) {
            if (!cellIsVacant(i, j)) {
...
function hasOppositeChecker(row, col, color) {
    return !((row > 7) || (row < 0) || (col > 7) || (col < 0) || (occupiedArray[row][col] === null) || (occupiedArray[row][col].classList.contains(color)));
}
...
function moveString(source, destination, player) {
    let position = "";
    let currentPlayer = source[0];
    let moveNumber1 = null;
    let moveNumber2 = null;
    for(let j = 2; j < 35; j++) {
...
function findJumpMove(source, destination, index, position) {
    ...
    let topLeftOffset = ((Math.floor((index - 2) / 4) + 1) % 2) + 4;
    let topRightOffset = ((Math.floor((index - 2) / 4) + 1) % 2) + 3;
    let bottomLeftOffset = (Math.floor((index - 2) / 4) % 2) + 3;
    let bottomRightOffset = (Math.floor(((index - 2) / 4)) % 2) + 4;
    let betweenBoard = source;
...
function appendMoves(movementString, player) {
    let table = document.getElementById("gameRecord");
    let row = table.insertRow(-1);
    let turnCell = row.insertCell(0);
    let playerCell = row.insertCell(1);
    let boardCell = row.insertCell(2);
    let moveCell = row.insertCell(3);
